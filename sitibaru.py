...
# PERBAIKAN: Menambahkan 'api_client' ke dalam ingatan (session state)
...
if start_btn:
    if not username or not api_key:
        ...
        try:
            # PERBAIKAN: Menyimpan client ke dalam session_state agar tidak terputus
            ...
            # Membuat sesi obrolan dengan menggunakan client yang sudah diamankan di session_state
    ...
    if prompt := st.chat_input("Ketik balasan Anda dalam bahasa Arab (atau alfabet/latin) di sini..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner(f"{tutor_choice} sedang mengetik balasan..."):
                try:
                    # Mengirim pesan dengan sesi yang aktif
